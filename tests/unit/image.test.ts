...
import { dummy_inspect_response, dummy_updated_inspect_response } from '../resources/dummy_inspect';

describe('Image Client', () => {
    let imageData = dummy_inspect_response;
    const MockDockerClient = {
        listImages: () => new Promise((resolve, reject) => {
        ...
        pull: (name: string) => {
            if (name === 'example:latest')
                imageData = dummy_updated_inspect_response;
        }
    }
